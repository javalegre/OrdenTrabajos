...
<div class="row no-margins">
        ...
        <h3 class="pull-right">Orden Trabajo Nº <?= $ordenTrabajo->id ?></h3>
        ...
        <tbody>
            <?php foreach ($ordenTrabajo['orden_trabajos_distribuciones'] as $distribucion): ?>
                <tr>
                    ...
                    <td><?= $distribucion->lote->has('sectore') ? h($distribucion->lote->sectore['nombre']) : '' ?></td>
                    <td class="derecha"><?= h($distribucion['superficie']) ?></td>
                    <?php
                        $total_superficie += $distribucion['superficie'];
                    ?>
<!--                    <td class="derecha">< ?= h($distribucion['importe']) ?></td>
                    < ?php
                        /* Calculo el total en $ de la Orden de Trabajo */
                        $total = $distribucion['superficie'] * $distribucion['importe'];
                    ?>
                    <td class="derecha">< ?= h($total) ?></td>-->
                </tr>
            <?php endforeach; ?>                            
        </tbody>
        <tfoot>
...
<?php if (count($ordenTrabajo['orden_trabajos_insumos']) > 0){ ?>
    Insumos a utilizar
    <table class="table table-bordered table-striped" cellspacing="0" width="100%">
        <thead class="blue-bg"><?= $this->Html->tableHeaders(['Producto', 'Lote', 'Un','Dosis','Cant.','Entregas','Devoluc.','Almacen']) ?></thead>
        <tbody>
            <?php foreach ($ordenTrabajo['orden_trabajos_insumos'] as $insumo): ?>
                <tr>
                    ...
                    <td class="derecha"><?= $insumo->entrega ? h($insumo->entrega) : '0' ?></td>
                    <td class="derecha"><?= $insumo->devolucion ? h($insumo->devolucion) : '0' ?></td>
...
<!--<table width="100%">
    <tr class="fondo">
        <td width="100%">
            <span>< ?= $this->Html->image(WWW_ROOT.'img'.DS.'OrdenTrabajoMapas'.DS.'OT-'.$ordenTrabajo->id.'-'.'mapa.png', ['width' => '100%']) ?></span>
        </td>
        <td width="50%">
            <span>< ?= $this->Html->image(WWW_ROOT.'img'.DS.'OrdenTrabajoMapas'.DS.'OT-'.$ordenTrabajo->id.'-'.'kml.png', ['width' => '100%']) ?></span>
        </td>
    </tr>
</table>-->
