...
$(document).ready(function() {
    /* Inicio el show */
        ...
        $('#cm_humedad').numeric();
        $('#cm_viento').numeric();
        $('#fecha-modal').mask('00/00/0000');
        
        /* Traigo todos los datos necesarios */
        const IniciarCertificacion = () => {
            let id = $('#id').val();
            fetch(`/orden-trabajos/certificarot/${ id }.json`)
                .then( res => res.json() )
                .then( data => {
    ...
    /* ---------------------------------------------------------------------- */
    /* Validación de la fecha de las Condiciones Meteorologicas               */
    /* ---------------------------------------------------------------------- */
...
var ContractorListEdit = function() {
            ...
            dataTableContractors.push(
                {
                    ...
                    moneda: distribucion[i]['moneda'].simbolo,
                    importe: distribucion[i].importe,
                    insumos: distribucion[i]['proyectos_labore'].insumos,
                    labor_id: distribucion[i]['proyectos_labore'].id,
                    importe_certificado: distribucion[i]['importe_certificado'],
                    id: distribucion[i]['id'],
                    tarifario: distribucion[i]['orden_trabajos_distribuciones_tarifario'] ? distribucion[i]['orden_trabajos_distribuciones_tarifario'] : ''
                }
            );
        }
    };
    ...
    var updateDataContractors = function(RowContractor) {
        ...
        $.each(DataContractors, function(index, row) {
            if (row.id == RowContractor.id) {
                ...
                DataContractors[index].id = RowContractor.tarifario;
                l_index = index;
                l_new = false;
                return true;
            }
    ...
    var initTable = function() {
        ...
        $table.on('click', 'td.details-control-contractors, .expand_machine', function() {
            ...
            if (data.insumos) {
                ...
                $('[data-toggle="tooltip"]').tooltip({trigger: 'hover', onclick: this.blur() });
                tr.addClass('shown');
                ...
                inputTypes: [
                    {
                        ...
                        column: 2,
                        type: "number",
                        class: "form-control input-small edit-input-inline"
        ...
        onUpdate: callbackEditTableOrdenes,
        errorClass: 'edit-input-error',
        nombre: 'TablaLabores',
        ...
        inputTypes: [
            { /* Labores */
                ...
                column: 2,
                type: "text",
                class: "input-small edit-input-inline select2"
            }, { /* Centro de Costos - Campañas */
                column: 3,
                type: "text"
            }, { /* Lotes */
                column: 4,
                ...
                column: 8,
                type: "list",
                class: "form-control input-small edit-input-inline select2"
            }, { /* Importe de Moneda */
                column: 9,
                type: "number"
            }
    ...
    var callbackEditTableOrdenes = function(table, aData, flagDelete, row) {
        ...
        .then( data => {
            if(data['status']=='success') {
                aData.certificadas = 0;
                aData.importe_certificado = 0;        
                table.row(row).data(aData);
                table.draw();
                
                var certificaciones = `<div class="ibox-content ibox-heading"><h3>Tiene ${ aData.has } hectareas sin certificar!</h3></div>`;
                $('#cantidad-certificaciones').html( certificaciones );
...
//                    $('.certificacion-notificacion').removeClass('hidden').addClass('hidden');
    ...
    const callBackMonedas = async (value, event) => {
        ...
        /* Busco la moneda de destino */ 
        var moneda_destino = monedas.filter(function(value) {
            return value.id == this; 
        }, value);
        
        /* 
         * Busco la fecha de ordenado, no sé porque no lo toma automaticamente
         * Creo que tiene que ver con el tema de las formato
         */
        var fecha_cotizacion = '';
        $.each(event.data.domTD, function (index, td) {
            if (index === 0) { /* Fecha labor */
                var inputField;
                if ($(td).find('input').length > 0) {
                    inputField = $(td).find('input');
                }
                fecha_cotizacion = moment($(inputField).val(), "DD/MM/YYYY").format("YYYY-MM-DD");
            }
        });
        
        let Cotizacion = await ObtenerCotizacion(fecha_cotizacion, row.moneda_ordenada , moneda_destino[0].nombre );
    ...
    var callbackEditandoTabla = function (table) {
        ...
        /* Si hay moneda en USD */
        if (row.moneda_id === 2) {
            var lt_disabled = { 4: 2, 5: 5};
        }
